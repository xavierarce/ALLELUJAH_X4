...
| `--sequentiel` | Un seul appel API à la fois, pour mesurer le gain des threads |
...
- Un **SIRET est ramené à ses 9 premiers chiffres**, son SIREN : c'est le seul
  identifiant que l'API sait rechercher à l'identique.
- Avec un identifiant la vérification est **exacte** ; sans, on cherche par nom
  et le résultat est **approché** (voir `score_correspondance`).
  ...
  "prospects": [
    {
      ...
      "message": "Cessation d'activité le 2025-09-19 (il y a 355 jours) — ne pas démarcher.",
...
Le verdict tient dans **deux champs**, et c'est voulu :

- `etat_activite` — `ACTIVE` / `CESSEE` / `INCONNU` : la réponse à la question
  du client ;
- `alerte` — **vide s'il n'y a rien à signaler**, sinon `CESSATION_RECENTE`,
  `CESSEE`, `ETAT_INCONNU`, `CORRESPONDANCE_INCERTAINE`, `INTROUVABLE`,
  `IDENTIFIANT_INVALIDE` ou `ERREUR_API`.

Le reste est du contexte : `message` (la phrase à lire avant de décrocher le
...
aval n'a jamais à tester la présence d'un champ.

### Extraire les prospects à ne pas démarcher

```bash
jq '.prospects[] | select(.alerte != "") | {nom_officiel, alerte, message}' resultats/rapport.json
```

Sans `jq`, en une ligne de Python :

```bash
python3 -c "import json; r=json.load(open('resultats/rapport.json')); print(*[p['nom_officiel'] or p['nom_saisi'] for p in r['prospects'] if p['alerte']], sep='\n')"
```
...
| Prospect fourni par SIRET | ramené à son SIREN, le seul que l'API sait chercher |
...
**Travail commun :** le contrat de sortie (les clés de `analyse.VERDICT_VIDE`) a
